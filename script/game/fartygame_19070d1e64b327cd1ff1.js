...
(function()
{
	...
	function GameLoader(onReady, onLoaded)
	{
		...
		spil.logoImage = this.loadImage("assets/saco.png");
	...
	{
		click:new Lang({en_GB:"点击", nl_NL:"点击"}),
		tap:new Lang({en_GB:"点击", nl_NL:"点击"}),
		best:new Lang({en_GB:"最高分数: %0%", nl_NL:"最高分数: %0%"}),
		fart:new Lang({en_GB:"%0%开始游戏", nl_NL:"%0%开始游戏"}),
		refart:new Lang({en_GB:"%0%重新开始", nl_NL:"%0%重新开始"}),
		activate:new Lang({en_GB:"%0%\n激活", nl_NL:"%0% IS\nACTIVEREN"}),
		ready:new Lang({en_GB:"准备!", nl_NL:"OPGELET!"}),
		interact:new Lang(null),
		belt:new Lang({en_GB:"财富腰带", nl_NL:"Ei-tastische Riem"}),
		beltsub:new Lang({en_GB:"获得更多的金蛋!", nl_NL:"Vindt meer gouden eieren!"}),
		magnet:new Lang({en_GB:"蛋蛋磁铁", nl_NL:"Ei Magneet"}),
		magnetsub:new Lang({en_GB:"吸引这些蛋蛋!", nl_NL:"Grijp die eieren!"}),
		helmet:new Lang({en_GB:"怪兽头盔", nl_NL:"Kaiju Helm"}),
		helmetsub:new Lang({en_GB:"抵挡一次伤害!", nl_NL:"Overleef een klap!"}),
		rexx:new Lang({en_GB:"方块龙"}),
		rexxsub:new Lang({en_GB:"使用方块龙模型!", nl_NL:"Speel als Rexx!"}),
		shop:new Lang({en_GB:"商店", nl_NL:"商店"}),
		start:new Lang({en_GB:"开始", nl_NL:"开始"}),
		back:new Lang({en_GB:"<菜单", nl_NL:"<菜单"}),
		menu:new Lang({en_GB:"菜单", nl_NL:"菜单"})
	};
	...
	Game.prototype.initStartScreen = function()
	{
		...
		{
			showmore();
			//window.open("http://static.egret-labs.org/h5game/gameList/game.html");
			//game.changeScreen(SCREEN_SHOP);
		};
		SCREEN_START.elements.push(ui);
	...
	Game.prototype.start = function()
	{
		...
		this.shareButton = new Button("return", this.images.menubutton, new LabelFormat(FONT, 70, false, false, "#FFFFFF", 18, "#032e3f"));
		this.shareButton.text = "分享";
		this.shareButton.verPct = -0.15;
		this.shareButton.hor = 0;
		this.shareButton.signals.onTap = function() { share();  game.changeScreen(SCREEN_START); };
		this.shareButton.signals.onUpdate = function()	{ this.visible = game.allowRestart; };
		//SCREEN_GAME.elements.push(this.shareButton); //decamincow
